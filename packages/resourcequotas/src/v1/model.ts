...
const getQuotaFieldMaskSchema: FieldMaskSchema = {
  parentFullName: {wire: 'parent_full_name'},
  parentSecurableType: {wire: 'parent_securable_type'},
  quotaName: {wire: 'quota_name'},
};

export function getQuotaFieldMask(...paths: string[]): FieldMask<GetQuota> {
  return FieldMask.build<GetQuota>(paths, getQuotaFieldMaskSchema);
}

// eslint-disable-next-line @typescript-eslint/naming-convention -- Proto-style nested message name.
const getQuota_ResponseFieldMaskSchema: FieldMaskSchema = {
  quotaInfo: {wire: 'quota_info', children: () => quotaInfoFieldMaskSchema},
};

// eslint-disable-next-line @typescript-eslint/naming-convention -- Proto-style nested message name.
export function getQuota_ResponseFieldMask(
  ...paths: string[]
): FieldMask<GetQuota_Response> {
  return FieldMask.build<GetQuota_Response>(
    paths,
    getQuota_ResponseFieldMaskSchema
  );
}

const listQuotasFieldMaskSchema: FieldMaskSchema = {
  maxResults: {wire: 'max_results'},
  pageToken: {wire: 'page_token'},
};

export function listQuotasFieldMask(...paths: string[]): FieldMask<ListQuotas> {
  return FieldMask.build<ListQuotas>(paths, listQuotasFieldMaskSchema);
}

// eslint-disable-next-line @typescript-eslint/naming-convention -- Proto-style nested message name.
const listQuotas_ResponseFieldMaskSchema: FieldMaskSchema = {
  nextPageToken: {wire: 'next_page_token'},
  quotas: {wire: 'quotas'},
};

// eslint-disable-next-line @typescript-eslint/naming-convention -- Proto-style nested message name.
export function listQuotas_ResponseFieldMask(
  ...paths: string[]
): FieldMask<ListQuotas_Response> {
  return FieldMask.build<ListQuotas_Response>(
    paths,
    listQuotas_ResponseFieldMaskSchema
  );
}

const quotaInfoFieldMaskSchema: FieldMaskSchema = {
  lastRefreshedAt: {wire: 'last_refreshed_at'},
  parentFullName: {wire: 'parent_full_name'},
  parentSecurableType: {wire: 'parent_securable_type'},
  quotaCount: {wire: 'quota_count'},
  quotaLimit: {wire: 'quota_limit'},
  quotaName: {wire: 'quota_name'},
};

export function quotaInfoFieldMask(...paths: string[]): FieldMask<QuotaInfo> {
  return FieldMask.build<QuotaInfo>(paths, quotaInfoFieldMaskSchema);
}
