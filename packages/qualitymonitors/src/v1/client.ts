...
export class Client {
   ...
   */
  async cancelRefresh(signal: AbortSignal | undefined, req: CancelRefresh, options?: Options): Promise<CancelRefresh_Response> {
   ...
   * this call was made.
   */
  async createMonitor(signal: AbortSignal | undefined, req: CreateMonitor, options?: Options): Promise<DataMonitorInfo> {
   ...
   */
  async deleteMonitor(signal: AbortSignal | undefined, req: DeleteMonitor, options?: Options): Promise<DeleteMonitor_Response> {
    const url = `${this.host}/api/2.1/unity-catalog/tables/${req.fullTableNameArg ?? ''}/monitor`;
   ...
   */
  async getMonitor(signal: AbortSignal | undefined, req: GetMonitor, options?: Options): Promise<DataMonitorInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/tables/${req.fullTableNameArg ?? ''}/monitor`;
    let resp: DataMonitorInfo | undefined;
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
   ...
   * Additionally, the call must be made from the workspace where the monitor was created.
   */
  async getRefresh(signal: AbortSignal | undefined, req: GetRefresh, options?: Options): Promise<RefreshInfo> {
   ...
   * Additionally, the call must be made from the workspace where the monitor was created.
   */
  async listRefreshes(signal: AbortSignal | undefined, req: ListRefreshes, options?: Options): Promise<ListRefreshes_Response> {
   ...
   */
  async regenerateDashboard(signal: AbortSignal | undefined, req: RegenerateDashboard, options?: Options): Promise<RegenerateDashboard_Response> {
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      const httpReq = buildHttpRequest('POST', url, callSignal, body);
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalRegenerateDashboard_ResponseSchema);
    };
    await execute(signal, call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
   ...
   * Additionally, the call must be made from the workspace where the monitor was created.
   */
  async runRefresh(signal: AbortSignal | undefined, req: RunRefresh, options?: Options): Promise<RefreshInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/tables/${req.fullTableNameArg ?? ''}/monitor/refreshes`;
   ...
   */
  async updateMonitor(signal: AbortSignal | undefined, req: UpdateMonitor, options?: Options): Promise<DataMonitorInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/tables/${req.fullTableNameArg ?? ''}/monitor`;
