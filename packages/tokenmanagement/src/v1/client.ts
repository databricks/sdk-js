...
export class TokenManagementClient {
  ...
  async createOnBehalfOfToken(
    ...
    const url = `${host}/api/2.0/token-management/on-behalf-of/tokens`;
  ...
  async deleteToken(
    ...
    const {host, workspaceId, httpClient} = await this.resolveConfig();
    const url = `${host}/api/2.0/token-management/tokens/${req.tokenId ?? ''}`;
  ...
  async listTokens(
    ...
    const {host, workspaceId, httpClient} = await this.resolveConfig();
    const url = `${host}/api/2.0/token-management/tokens`;
