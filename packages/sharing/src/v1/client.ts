...
export class Client {
  ...
  async listRecipients(
    ...
    if (req.parentRecipientName !== undefined) {
      params.append('parent_recipient_name', req.parentRecipientName);
    }
    const query = params.toString();
    const fullUrl = query !== '' ? `${url}?${query}` : url;
  ...
  /**
   * Updates an existing federation policy for an OIDC_RECIPIENT.
   * The caller must be the owner of the recipient.
   */
  async updateFederationPolicy(
    req: UpdateFederationPolicyRequest,
    options?: CallOptions
  ): Promise<FederationPolicy> {
    const url = `${this.host}/api/2.0/data-sharing/recipients/${req.recipientName ?? ''}/federation-policies/${req.name ?? ''}`;
    const params = new URLSearchParams();
