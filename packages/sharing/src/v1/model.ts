...
export enum DeltaSharingAuthenticationType {
  ...
  /**
   * Email-based sharing. The recipient is invited by email and chooses an authentication method
   * (TOKEN or DATABRICKS) when accepting the invite.
   */
  EMAIL = 'EMAIL',
}
...
export interface CreateProviderRequest {
  ...
  emailRecipientId?: string | undefined;
}
...
export interface CreateRecipientRequest {
  ...
  id?: string | undefined;
  /** The invited email address. This field is only present when the __authentication_type__ is **EMAIL**. The 320-character cap follows RFC 5321 (64-octet local-part + `@` + 255-octet domain). */
  email?: string | undefined;
  /** Authentication types the invitee may choose from when activating. Each value must be TOKEN or DATABRICKS. Empty means all supported types are allowed. This field is only present when the __authentication_type__ is **EMAIL**. */
  allowedAcceptanceTypes?: DeltaSharingAuthenticationType[] | undefined;
  /** Optional allowlist of the data recipient's global metastore IDs permitted to accept the invite with the DATABRICKS acceptance type. Each ID is of the form __cloud__:__region__:__metastore-uuid__. Empty means any metastore is allowed. If non-empty, __allowed_acceptance_types__ must include **DATABRICKS**. This field is only present when the __authentication_type__ is **EMAIL**. */
  allowedDataRecipientGlobalMetastoreIds?: string[] | undefined;
}
...
export interface CreateShareRequest {
   ...
   * [Create,Update:IGN]
   */
  serverlessBudgetPolicyId?: string | undefined;
}
...
export interface GetActivationUrlInfoRequest {
  ...
  activationUrl?: string | undefined;
  /** The provider metastore ID. Optional for now; used as a routing hint when present and principal context */
  metastoreId?: string | undefined;
}
...
export interface ListRecipientsRequest {
  ...
  /** Opaque pagination token to go to next page based on previous query. */
  pageToken?: string | undefined;
  /** If set, the response is scoped to the child recipients of the named parent recipient. Child recipients are otherwise hidden from the default list response. Only applicable to email recipient parents — non-email recipients have no children to scope to. */
  parentRecipientName?: string | undefined;
}
...
export interface Table {
   ...
   * Used for open sharing to indicate how the table can be accessed.
   */
...
export interface UpdateFederationPolicyRequest {
   ...
   * in the existing policy. Example value: 'comment,oidc_policy.audiences'.
   */
...
export const unmarshalProviderInfoSchema: z.ZodType<ProviderInfo> = z
  .object({
    ...
    email_recipient_id: z.string().optional(),
  })
  .transform(d => ({
    name: d.name,
    ...
    emailRecipientId: d.email_recipient_id,
  }));
...
export const unmarshalRecipientInfoSchema: z.ZodType<RecipientInfo> = z
  .object({
    ...
    allowed_acceptance_types: z
      .array(z.enum(DeltaSharingAuthenticationType))
      .optional(),
    allowed_data_recipient_global_metastore_ids: z.array(z.string()).optional(),
  })
  .transform(d => ({
    name: d.name,
    authenticationType: d.authentication_type,
    ...
    id: d.id,
    email: d.email,
    allowedAcceptanceTypes: d.allowed_acceptance_types,
    allowedDataRecipientGlobalMetastoreIds:
      d.allowed_data_recipient_global_metastore_ids,
  }));
...
export const unmarshalReplicationConfigSchema: z.ZodType<ReplicationConfig> = z
  .object({
...
export const unmarshalShareInfoSchema: z.ZodType<ShareInfo> = z
  .object({
    ...
    replication_config: z
      .lazy(() => unmarshalReplicationConfigSchema)
      .optional(),
    ...
    updated_by: z.string().optional(),
    storage_location: z.string().optional(),
    serverless_budget_policy_id: z.string().optional(),
  })
  .transform(d => ({
    name: d.name,
    owner: d.owner,
    ...
    replicationConfig: d.replication_config,
    objects: d.objects,
    createdAt: d.created_at,
    createdBy: d.created_by,
    updatedAt: d.updated_at,
    updatedBy: d.updated_by,
    storageLocation: d.storage_location,
    serverlessBudgetPolicyId: d.serverless_budget_policy_id,
  }));
...
export const marshalCreateProviderRequestSchema: z.ZodType = z
  .object({
    ...
    emailRecipientId: z.string().optional(),
  })
  .transform(d => ({
    name: d.name,
    ...
    email_recipient_id: d.emailRecipientId,
  }));
...
export const marshalCreateRecipientRequestSchema: z.ZodType = z
  .object({
    ...
    id: z.string().optional(),
    email: z.string().optional(),
    allowedAcceptanceTypes: z
      .array(z.enum(DeltaSharingAuthenticationType))
      .optional(),
    allowedDataRecipientGlobalMetastoreIds: z.array(z.string()).optional(),
  })
  .transform(d => ({
    name: d.name,
    authentication_type: d.authenticationType,
    ...
    id: d.id,
    email: d.email,
    allowed_acceptance_types: d.allowedAcceptanceTypes,
    allowed_data_recipient_global_metastore_ids:
      d.allowedDataRecipientGlobalMetastoreIds,
  }));
...
export const marshalCreateShareRequestSchema: z.ZodType = z
  .object({
    ...
    replicationConfig: z.lazy(() => marshalReplicationConfigSchema).optional(),
    ...
    serverlessBudgetPolicyId: z.string().optional(),
  })
  .transform(d => ({
    name: d.name,
    owner: d.owner,
    ...
    replication_config: d.replicationConfig,
    objects: d.objects,
    created_at: d.createdAt,
    created_by: d.createdBy,
    updated_at: d.updatedAt,
    ...
    serverless_budget_policy_id: d.serverlessBudgetPolicyId,
  }));
...
export const marshalReplicationConfigSchema: z.ZodType = z
  .object({
    enabled: z.boolean().optional(),
  })
  .transform(d => ({
