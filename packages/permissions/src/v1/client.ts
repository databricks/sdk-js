...
export class Client {
  ...
  async getObjectPermissions(signal: AbortSignal | undefined, req: GetObjectPermissions, options?: Options): Promise<PermissionsResponse> {
  ...
  async getPermissionLevels(signal: AbortSignal | undefined, req: GetPermissionLevels, options?: Options): Promise<GetPermissionLevels_Response> {
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      ...
      const httpReq = buildHttpRequest('GET', url, headers, callSignal);
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalGetPermissionLevels_ResponseSchema);
    };
    await execute(signal, call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
  ...
  async setObjectPermissions(signal: AbortSignal | undefined, req: SetObjectPermissions, options?: Options): Promise<PermissionsResponse> {
    const url = `${this.host}/api/2.0/permissions/${req.requestObjectType ?? ''}/${req.requestObjectId ?? ''}`;
  ...
  async updateObjectPermissions(signal: AbortSignal | undefined, req: UpdateObjectPermissions, options?: Options): Promise<PermissionsResponse> {
    const url = `${this.host}/api/2.0/permissions/${req.requestObjectType ?? ''}/${req.requestObjectId ?? ''}`;
