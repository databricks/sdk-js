...
export class Client {
   ...
   */
  async createVolume(req: CreateVolumeRequest, options?: CallOptions): Promise<VolumeInfo> {
   ...
   */
  async deleteVolume(req: DeleteVolumeRequest, options?: CallOptions): Promise<DeleteVolumeRequest_Response> {
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      ...
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalDeleteVolumeRequest_ResponseSchema);
    };
    await executeCall(call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
   ...
   * privilege on the parent catalog and the **USE_SCHEMA** privilege on the parent schema.
   */
  async getVolume(req: GetVolumeRequest, options?: CallOptions): Promise<VolumeInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/volumes/${req.fullNameArg ?? ''}`;
   ...
   */
  async listVolumes(req: ListVolumesRequest, options?: CallOptions): Promise<ListVolumesRequest_Response> {
    const url = `${this.host}/api/2.1/unity-catalog/volumes`;
    const params = new URLSearchParams();
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      ...
      const httpReq = buildHttpRequest('GET', fullUrl, headers, callSignal);
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalListVolumesRequest_ResponseSchema);
    };
    await executeCall(call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
  ...
  async *listVolumesIter(req: ListVolumesRequest, options?: CallOptions): AsyncGenerator<VolumeInfo> {
   ...
   */
  async updateVolume(req: UpdateVolumeRequest, options?: CallOptions): Promise<VolumeInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/volumes/${req.fullNameArg ?? ''}`;
