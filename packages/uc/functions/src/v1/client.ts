...
export class Client {
   ...
   */
  async createFunction(req: CreateFunctionRequest, options?: CallOptions): Promise<FunctionInfo> {
   ...
   */
  async deleteFunction(req: DeleteFunctionRequest, options?: CallOptions): Promise<DeleteFunctionRequest_Response> {
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      ...
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalDeleteFunctionRequest_ResponseSchema);
    };
    await executeCall(call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
   ...
   */
  async getFunction(req: GetFunctionRequest, options?: CallOptions): Promise<FunctionInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/functions/${req.fullNameArg ?? ''}`;
    const params = new URLSearchParams();
   ...
   */
  async listFunctions(req: ListFunctionsRequest, options?: CallOptions): Promise<ListFunctionsRequest_Response> {
    const url = `${this.host}/api/2.1/unity-catalog/functions`;
    const params = new URLSearchParams();
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      ...
      const httpReq = buildHttpRequest('GET', fullUrl, headers, callSignal);
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalListFunctionsRequest_ResponseSchema);
    };
    await executeCall(call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
  ...
  async *listFunctionsIter(req: ListFunctionsRequest, options?: CallOptions): AsyncGenerator<FunctionInfo> {
   ...
   */
  async updateFunction(req: UpdateFunctionRequest, options?: CallOptions): Promise<FunctionInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/functions/${req.fullNameArg ?? ''}`;
