...
export class ResourceQuotasClient {
  ...
  async getQuota(
    ...
    const url = `${host}/api/2.1/unity-catalog/resource-quotas/${req.parentSecurableType ?? ''}/${req.parentFullName ?? ''}/${req.quotaName ?? ''}`;
  ...
  async listQuotas(
    ...
    const {host, workspaceId, httpClient} = await this.resolveConfig();
    const url = `${host}/api/2.1/unity-catalog/resource-quotas/all-resource-quotas`;
