...
export class Client {
  ...
  async createDirectory(
    ...
    const url = `${this.host}/api/2.0/fs/directories${(req.directoryPath ?? '').split('/').map(encodeURIComponent).join('/')}`;
    let resp: CreateDirectoryResponse | undefined;
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      const httpReq = buildHttpRequest('PUT', url, callSignal);
      const respBody = await executeHttpCall({
        request: httpReq,
        httpClient: this.httpClient,
        logger: this.logger,
  ...
  async deleteFile(
    ...
    const url = `${this.host}/api/2.0/fs/files${(req.filePath ?? '').split('/').map(encodeURIComponent).join('/')}`;
  ...
  async getDirectoryMetadata(
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      const httpReq = buildHttpRequest('head', url, callSignal);
      const respBody = await executeHttpCall({
        request: httpReq,
        httpClient: this.httpClient,
        logger: this.logger,
  ...
  async getFileMetadata(
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      const httpReq = buildHttpRequest('head', fullUrl, callSignal);
      const respBody = await executeHttpCall({
        request: httpReq,
        httpClient: this.httpClient,
        logger: this.logger,
