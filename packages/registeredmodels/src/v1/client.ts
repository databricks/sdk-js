...
export class Client {
   ...
   */
  async createRegisteredModel(signal: AbortSignal | undefined, req: CreateRegisteredModel, options?: Options): Promise<RegisteredModelInfo> {
   ...
   */
  async deleteModelVersion(signal: AbortSignal | undefined, req: DeleteModelVersion, options?: Options): Promise<DeleteModelVersion_Response> {
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      ...
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalDeleteModelVersion_ResponseSchema);
    };
    await execute(signal, call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
   ...
   * privilege on the parent catalog and the **USE_SCHEMA** privilege on the parent schema.
   */
  async deleteRegisteredModel(signal: AbortSignal | undefined, req: DeleteRegisteredModel, options?: Options): Promise<DeleteRegisteredModel_Response> {
    const url = `${this.host}/api/2.1/unity-catalog/models/${req.fullNameArg ?? ''}`;
    let resp: DeleteRegisteredModel_Response | undefined;
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      const httpReq = buildHttpRequest('DELETE', url, callSignal);
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalDeleteRegisteredModel_ResponseSchema);
    };
    await execute(signal, call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
   ...
   * privilege on the parent catalog and the **USE_SCHEMA** privilege on the parent schema.
   */
  async deleteRegisteredModelAlias(signal: AbortSignal | undefined, req: DeleteRegisteredModelAlias, options?: Options): Promise<DeleteRegisteredModelAlias_Response> {
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      const httpReq = buildHttpRequest('DELETE', url, callSignal);
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalDeleteRegisteredModelAlias_ResponseSchema);
    };
    await execute(signal, call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
   ...
   */
  async getModelVersion(signal: AbortSignal | undefined, req: GetModelVersion, options?: Options): Promise<ModelVersionInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/models/${req.fullNameArg ?? ''}/versions/${String(req.versionArg ?? '')}`;
   ...
   * on the parent schema.
   */
  async getModelVersionByAlias(signal: AbortSignal | undefined, req: GetModelVersionByAlias, options?: Options): Promise<ModelVersionInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/models/${req.fullNameArg ?? ''}/aliases/${req.aliasArg ?? ''}`;
    const params = new URLSearchParams();
    if (req.includeAliases !== undefined) {
      params.append('include_aliases', String(req.includeAliases));
   ...
   * on the parent schema.
   */
  async getRegisteredModel(signal: AbortSignal | undefined, req: GetRegisteredModel, options?: Options): Promise<RegisteredModelInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/models/${req.fullNameArg ?? ''}`;
    const params = new URLSearchParams();
    if (req.includeAliases !== undefined) {
      params.append('include_aliases', String(req.includeAliases));
   ...
   */
  async listModelVersions(signal: AbortSignal | undefined, req: ListModelVersions, options?: Options): Promise<ListModelVersions_Response> {
  ...
  async *listModelVersionsIter(signal: AbortSignal | undefined, req: ListModelVersions, options?: Options): AsyncGenerator<ModelVersionInfo> {
   ...
   * Clients must continue reading pages until next_page_token is absent, which is the only indication that the end of results has been reached.
   */
  async listRegisteredModels(signal: AbortSignal | undefined, req: ListRegisteredModels, options?: Options): Promise<ListRegisteredModels_Response> {
    const url = `${this.host}/api/2.1/unity-catalog/models`;
    const params = new URLSearchParams();
    ...
    const call: Call = async (callSignal?: AbortSignal): Promise<void> => {
      const httpReq = buildHttpRequest('GET', fullUrl, callSignal);
      const respBody = await executeHttpCall({request: httpReq, httpClient: this.httpClient, logger: this.logger});
      resp = parseResponse(respBody, unmarshalListRegisteredModels_ResponseSchema);
    };
    await execute(signal, call, options);
    if (resp === undefined) {
      throw new Error('API call completed without a result.');
  ...
  async *listRegisteredModelsIter(signal: AbortSignal | undefined, req: ListRegisteredModels, options?: Options): AsyncGenerator<RegisteredModelInfo> {
   ...
   * privilege on the parent catalog and the **USE_SCHEMA** privilege on the parent schema.
   */
  async setRegisteredModelAlias(signal: AbortSignal | undefined, req: SetRegisteredModelAlias, options?: Options): Promise<RegisteredModelAliasInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/models/${req.fullNameArg ?? ''}/aliases/${req.aliasArg ?? ''}`;
   ...
   */
  async updateModelVersion(signal: AbortSignal | undefined, req: UpdateModelVersion, options?: Options): Promise<ModelVersionInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/models/${req.fullNameArg ?? ''}/versions/${String(req.versionArg ?? '')}`;
   ...
   */
  async updateRegisteredModel(signal: AbortSignal | undefined, req: UpdateRegisteredModel, options?: Options): Promise<RegisteredModelInfo> {
    const url = `${this.host}/api/2.1/unity-catalog/models/${req.fullNameArg ?? ''}`;
