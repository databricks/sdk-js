...
### 2. `ActivityType` enum — `model.ts:47-62`
...
### 3. `RegistryEmailSubscriptionType` enum — `model.ts:106-111`
...
### 4. `RegistryWebhookEvent` enum — `model.ts:113-126`
...
### 5. `Activity` vs `CommentObject` vs `TransitionRequest` — `model.ts:150, 227, 1015`
...
- **Why weird:** `Object` is the most generic suffix possible in TS
  ...
  (#5), this is a textbook bad name.
- **Category:** 1 (vague `Object` suffix), 20 (type-suffix tautology).
- **Suggested name:** `Comment` — or fold into `Activity` per #5.
...
### 8. `TransitionModelVersionStageDatabricksRequest` — `model.ts:981`
...
### 9. `GetRegisteredModelDatabricksRequest` request DTO — `model.ts:542`
...
### 10. `client.listTransitionsRequest` method vs `ListTransitionRequest`
...
### 11. `RegistryWebhook` vs `Webhook` — `model.ts:787`
...
### 14. `stage: string` field on `ApproveTransitionRequest`,
...
### 15. `currentStage: string` field on `ModelVersion`,
   `ModelVersionDatabricks` — `model.ts:670, 701`
- **Why weird:** Same as #14 — typed as `string`, valid values
...
### 16. `fromStage`, `toStage` fields — `model.ts:171, 183, 248, 260, 1036, 1048`
...
### 17. `Databricks` as a suffix is overused
- **Why weird:** Distinct type names still end in `Databricks` (see #7):
...
### 18. `getRegisteredModelDatabricks` client method — `client.ts:416`
...
### 19. `transitionModelVersionStageDatabricks` client method —
   `client.ts:615`
- **Why weird:** Same `Databricks` mid-position leak as #18. The token
...
- **Suggested name:** `transitionModelVersionStage` (drop the infix) or
  ...
  overlap (see #8).
- **Rationale:** Same as #18 — the infix encodes a generator-side
  distinction that callers do not need.

## Medium severity

### 20. `tags?: ModelVersionTag[] | undefined` and
...
### 21. `openRequests: Activity[]` on `ModelVersionDatabricks` — `model.ts:716`
...
### 22. `requests: Activity[]` on list-transition-requests response —
   `model.ts:655`
...
- **Rationale:** Same as #21 — type contradicts domain because of the
  identical-shape problem (#5).

### 23. `registeredModelDatabricks: RegisteredModelDatabricks` —
   `model.ts:549`
- **Why weird:** Field name *is* the type name verbatim. The `Databricks`
  suffix problem (#7) cascades into the field name.
...
### 24. `modelVersionDatabricks: ModelVersionDatabricks` — `model.ts:1007`
- **Why weird:** Same as #23 for `ModelVersionDatabricks`.
- **Category:** 20.
- **Suggested name:** `modelVersion: ModelVersion`.
- **Rationale:** Same.

### 25. `getLatestVersions` / `GetLatestVersionsRequest` — `client.ts:917`,
   `model.ts:501`
...
### 26. `pageToken`, `nextPageToken`, `maxResults` — `model.ts:584, 586,
...
### 27. `newName: string` on `RenameRegisteredModelRequest` — `model.ts:862`
...
### 30. Acronym casing inside doc-comments
