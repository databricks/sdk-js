...
### 1. Method naming wildly inconsistent: 28 of 30 methods are `genieXxx`, 2 are bare — `src/v1/client.ts:133,1121`
...
### 2. `genieTrashSpace` is the only delete-style method named `Trash` — `src/v1/client.ts:1099`
...
### 3. `genieExecuteMessageAttachmentQuery` vs `genieGetMessageAttachmentQueryResult` vs `genieGenerateDownloadFullQueryResult` — three different verbs for retrieving the same data — `src/v1/client.ts:328,605,422`
...
### 5. `EvaluationStatusType` — type name has redundant `Type` suffix — `src/v1/model.ts:533`
...
### 6. `DatabricksServiceExceptionProto` — proto-arch-leak: `Service` mid + `Proto` suffix — `src/v1/model.ts:828`
...
### 7. `GenieStartConversationMessageRequest` — type name conflates `Conversation` and `Message` — `src/v1/model.ts:1483`
...
### 8. `GenieAttachment.attachment` discriminated-union field has the same name as its parent — `src/v1/model.ts:888`
...
### 9. `GenieEvalRunResponse` is the entity type, not just a "response" — `src/v1/model.ts:1085`
...
### 10. `GenieGenerateDownloadFullQueryResultResponse` — type name is a sentence — `src/v1/model.ts:1145`
...
### 11. `GenieGenerateDownloadFullQueryResultRequest` / `GenieGetDownloadFullQueryResultRequest` — pairs spell out long ladder of nouns — `src/v1/model.ts:1134,1161`
- **Why weird:** Same as #10 — these are the request twins. The phrase "Full Query Result" is also under-qualified: it distinguishes from "partial" (a `GetMessageAttachmentQueryResult` is also a full result, just inline).
...
### 12. `Result` type name — too generic — `src/v1/model.ts:1589`
...
### 13. `GenieSuggestedQuestionsAttachment` — payload type name redundant with parent — `src/v1/model.ts:903`
...
### 14. `genieCreateConversationMessageWaiter` and `genieStartConversationWaiter` — `Waiter` suffix — `src/v1/client.ts:196,1072`
...
### 15. `GenieCreateConversationMessage` — verb chain `Create + Conversation + Message` — `src/v1/model.ts:938, client.ts:165`
...
### 16. `genieListConversationComments` returns `comments` — plural matches but parent path drops "Message" — `src/v1/client.ts:735, model.ts:1255`
...
### 17. `genieGetQueryResultByAttachment` — `By` clause is Java/Spring-style — `src/v1/client.ts:667`
...
### 18. `pageSize` / `pageToken` casing — `src/v1/model.ts:1248,1250,1266,1268,...`
