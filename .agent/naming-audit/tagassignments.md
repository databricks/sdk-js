...
### 7. `pageSize` here vs. `maxResults` in sister `entitytagassignments` — `src/v1/model.ts:35` vs. `entitytagassignments/src/v1/model.ts:68`
...
### 8. `tagKey` and `tagValue` on `TagAssignment` — `src/v1/model.ts:52,54`
...
### 9. `ListTagAssignmentsRequest.entityType` / `entityId` are functionally required but typed optional — `src/v1/model.ts:31,33` and used in URL at `client.ts:141`
...
### 10. `ListTagAssignmentsRequest` (plural) vs. `TagAssignment` (singular) — `src/v1/model.ts:29` vs. `src/v1/model.ts:46`
...
### 12. `Call` type and `call` variable — `src/v1/client.ts:74,98,118,152,201` and `src/v1/utils.ts:27`
...
### 13. URL composition with `req.entityType ?? ''` etc. — `src/v1/client.ts:97,116,141,192`
...
- **Rationale:** See #9. Generator-wide concern.

### 14. `respBody` (bytes) vs. `resp` (parsed) — `src/v1/client.ts:78-83,122-128,156-161,211-216`
...
### 15. `httpReq` local variable — `src/v1/client.ts:77,101,121,155,204`
...
### 16. `pageReq` clone variable in paginated list — `src/v1/client.ts:174`
...
### 18. `buildHttpRequest` is just object-spread — `src/v1/utils.ts:96`
...
### 19. `AuthHttpClient` — `src/v1/transport.ts:43`
...
### 20. `TimeoutHttpClient` — `src/v1/transport.ts:61`
- **Why weird:** Same wrapper-name pattern as #19. JSDoc line 60: "Wraps an HttpClient and applies a default timeout to requests." Name encodes the wrapping mechanism (`HttpClient` suffix) plus the cross-cutting concern (`Timeout` prefix). Reads as `<concern><wrapped-type>` — a classic Decorator naming tell.
