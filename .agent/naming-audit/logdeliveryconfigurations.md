...
### M7. `Client` class is unprefixed — `client.ts:46`
...
### M8. Client method names embed the noun three times — `client.ts:90,122,150,214`
...
### M9. `updateLogDeliveryConfiguration` does not actually "update" — it only patches status — `client.ts:209-243`
...
### M10. `listLogDeliveryConfiguration` — singular noun on a method returning multiple — `client.ts:150,192`
...
### M11. `ListLogDeliveryConfiguration` (request type) is singular — `model.ts:141`
- **File:** `model.ts:141-155`.
- **Category:** 9 (singular/plural mismatch).
- **Why weird:** Same issue as M10 for the request DTO. The interface name says "List one configuration" but the method actually lists many. The class-level JSDoc says "List Log Delivery Configuration" (singular). Compare with `budgets.ListBudgetConfigurations` (plural) at `packages/budgets/src/v1/model.ts`.
...
### M12. `logDeliveryStatus` field vs `LogDeliveryStatus` type vs `LogDeliveryStatusEnum` enum — three identifiers conflated — `model.ts:117,205,217`
...
### M13. `creationTime` / `updateTime` — verb-tense inconsistency, type misleads as ISO timestamp — `model.ts:113-115,201-203`
...
### M14. `deliveryStartTime: string` for YYYY-MM — misleading type — `model.ts:109,197`
...
### M15. `workspaceIdsFilter` — pluralised collection name mixed with `Filter` suffix — `model.ts:105,193`
...
### M17. `host` field on `Client` lacks domain context — `client.ts:47`
...
### L18. `LogDeliveryType` enum values `BILLABLE_USAGE` / `AUDIT_LOGS` — singular/plural mismatch — `model.ts:58-60`
- **File:** `model.ts:56-61`.
...
### L19. `LogDeliveryOutputFormat.CSV` / `.JSON` — acronym casing OK but enum is binary, no need — `model.ts:25-27`
- **File:** `model.ts:23-28`.
...
### L20. `LogDeliveryConfigStatus.ENABLED` / `.DISABLED` enum-member doc strings are tautological — `model.ts:13-16`
- **File:** `model.ts:13-16`.
- **Category:** 1 (vague).
...
### L21. `LogDeliveryStatusEnum.NOT_FOUND` is a confusing terminal state — `model.ts:48-49`
- **File:** `model.ts:48-49`.
...
### L22. `PACKAGE_SEGMENT` constant — `client.ts:41`
...
### L23. `httpClient: HttpClient` field — type-suffix tautology — `client.ts:51`
...
### L24. `req` / `resp` / `opts` / `httpReq` abbreviations — `client.ts:91,99,103,127,153,170,193,196,215,223`
...
### L25. `pageReq` local in `listLogDeliveryConfigurationIter` — `client.ts:196`
- **File:** `client.ts:196`.
...
### L26. `executeCall` / `executeHttpCall` near-duplicate names — `utils.ts:26,65`
...
### L27. `HttpCallOptions` — `utils.ts:15`
- **File:** `utils.ts:15-19`.
...
### O28. `flattenQueryParams` is exported but unused — `utils.ts:123`
...
### O29. `parseResponse` is unused — `utils.ts:113`
...
### O30. `marshalRequest` is used twice — `client.ts:95,219`
Used for `createLogDeliveryConfiguration` and `updateLogDeliveryConfiguration`. Not dead.

### O31. `accountId` URL fallback — `client.ts:94,126,154,218`
...
### O32. JSDoc artefacts (`* *`, `<Databricks>` template) — `model.ts:6,20,32,53,64,77,120,137,165,225`
