...
### 12. `PACKAGE_SEGMENT` — `src/v1/client.ts:33`
...
### 13. `userAgent` field — `src/v1/client.ts:45`
...
### 14. `Call` type and `call` variable — `src/v1/client.ts:77-87, 101-110, 121-131, 157-173`
...
### 15. `req.parent ?? ''` / `req.name ?? ''` / `req.catalogConfig?.name ?? ''` URL composition — `src/v1/client.ts:74,100,119,148`
...
### 16. `respBody` vs `resp` variables — `src/v1/client.ts:81-86, 125-130, 167-172`
...
### 17. `httpReq` local variable — `src/v1/client.ts:80,104,124,160`
...
### 18. Wire/TS divergence is heavy
...
### 21. `dataclassification` lowercase package name
