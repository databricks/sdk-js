...
### 6. `CreateCatalogConfigRequest` / `DeleteCatalogConfigRequest` / `GetCatalogConfigRequest` / `UpdateCatalogConfigRequest` — `src/v1/model.ts:65,76,82,93`
...
### 9. `PACKAGE_SEGMENT` — `src/v1/client.ts:33`
...
### 10. `userAgent` field — `src/v1/client.ts:45`
...
### 11. `Call` type and `call` variable — `src/v1/client.ts:77-87, 101-110, 121-131, 157-173`
...
### 12. `req.parent ?? ''` / `req.name ?? ''` / `req.catalogConfig?.name ?? ''` URL composition — `src/v1/client.ts:74,100,119,148`
...
### 13. `respBody` vs `resp` variables — `src/v1/client.ts:81-86, 125-130, 167-172`
...
### 14. `httpReq` local variable — `src/v1/client.ts:80,104,124,160`
...
### 15. Wire/TS divergence is heavy
...
### 18. `dataclassification` lowercase package name
