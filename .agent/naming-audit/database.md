...
### 8. `DatabaseInstanceRole_Attributes` vs `DatabaseInstanceRole.attributes` vs `DatabaseInstanceRole.effectiveAttributes` — `src/v1/model.ts:354,360,372`
...
### 9. `databaseCatalogs` plural field on `ListDatabaseCatalogsResponse` vs `syncedTables` (not `syncedDatabaseTables`) plural field on `ListSyncedDatabaseTablesResponse` — `src/v1/model.ts:498,542`
...
### 10. `DatabaseInstance.uid` and `DatabaseInstance.name` — both identifiers — `src/v1/model.ts:197,199`
...
### 11. `DatabaseInstance.creator` typed as `string` — `src/v1/model.ts:201`
...
### 12. `DatabaseInstance.capacity: string` typed as a free-form string but doc constrains it — `src/v1/model.ts:211`
...
### 13. `DatabaseInstance.pgVersion` casing — `src/v1/model.ts:209`
...
### 14. `DatabaseInstance.readWriteDns` / `readOnlyDns` — `src/v1/model.ts:203,250`
...
### 15. `DatabaseInstance.stopped` / `effectiveStopped` as a boolean toggle for state — `src/v1/model.ts:219,225`
...
### 16. `DatabaseInstance.nodeCount` is described as primary+secondaries — `src/v1/model.ts:230`
...
### 17. `DatabaseInstance.enableReadableSecondaries` boolean toggle naming — `src/v1/model.ts:239`
...
### 18. `DatabaseInstance.parentInstanceRef` / `childInstanceRefs` — `src/v1/model.ts:270,275`
...
### 19. `DatabaseInstanceRef.lsn` field — `src/v1/model.ts:323`
...
### 20. `DatabaseInstanceRef.branchTime` field — `src/v1/model.ts:342`
...
### 21. `DatabaseCatalog.uid` field with no doc — `src/v1/model.ts:185`
...
### 22. `DatabaseCatalog.createDatabaseIfNotExists` field — `src/v1/model.ts:186`
...
### 23. `DatabaseCatalog.databaseInstanceName` / `databaseName` — `src/v1/model.ts:182-184`
...
### 24. `DatabaseTable.name: string` "Full three-part (catalog, schema, table) name" — `src/v1/model.ts:380`
...
### 25. `SyncedDatabaseTable` vs `DatabaseTable` — overlapping concepts — `src/v1/model.ts:378,590`
...
### 26. `SyncedTableSpec.timeseriesKey` casing — `src/v1/model.ts:731`
...
### 27. `SyncedTableSpec.sourceTableFullName` vs `DatabaseTable.name` (also a full name) — `src/v1/model.ts:727,380`
...
- **Rationale:** Pair with #24.

### 28. `SyncedTableSpec.createDatabaseObjectsIfMissing` — `src/v1/model.ts:744`
- **Why weird:** Similar SQL-DDL leak as #22. Boolean named after a clause. Also doc at SyncedDatabaseTable.logicalDatabaseName references "the `create_database_objects_is_missing` field in `spec`" — that's a typo (`is_missing` vs `if_missing`) showing the field name fluctuates even in docs.
- **Category:** 14 (SQL-style name), 6 (typo in cross-reference).
- **Suggested name:** `ensureDatabaseAndSchema`.
- **Rationale:** Internal consistency.

### 29. `SyncedTableStatus.detailedState: SyncedTableState` — `src/v1/model.ts:759`
...
### 30. `SyncedTableStatus.detailedStatus` and its `*Status` sub-variants form a "status.status.status" chain — `src/v1/model.ts:763`
...
### 31. `SyncedTableStatus.lastSync: SyncedTablePosition` — `src/v1/model.ts:795`
...
### 32. `SyncedTablePipelineProgress.latestVersionCurrentlyProcessing` — `src/v1/model.ts:678`
...
### 33. `SyncedTablePipelineProgress.syncedRowCount` / `totalRowCount` / `syncProgressCompletion` / `estimatedCompletionTimeSeconds` — `src/v1/model.ts:680-686`
...
### 34. `SyncedTablePipelineProgress.syncProgressCompletion: number` doc says "a number between 0 and 1" — `src/v1/model.ts:683-684`
...
- **Rationale:** Same as #33 plus a separate concern about the value range.

### 35. `DeltaTableSyncInfo` is the only `*SyncInfo` type and the only `Delta*` type — `src/v1/model.ts:436`
...
### 36. `GenerateDatabaseCredentialRequest.claims` (plural `RequestedClaims[]`) but `RequestedClaims` is itself plural — `src/v1/model.ts:463,577`
...
### 37. `GenerateDatabaseCredentialRequest.requestId` — `src/v1/model.ts:453`
...
### 38. `GenerateDatabaseCredentialRequest.instanceNames` is "instance_names or claims" — `src/v1/model.ts:454-455`
...
### 39. `DeleteDatabaseInstanceRequest.purge` field is documented as deprecated — `src/v1/model.ts:411-415`
...
### 40. `NewPipelineSpec` type and `newPipelineSpec` field — `src/v1/model.ts:551,753`
...
### 41. Repeated generic suffixes `*Spec` and `*Info` across the package — `src/v1/model.ts:436,551,575,723`
...
- **Suggested name:** `SyncedTableSpec` → `SyncedTableConfig` (still generic) or better `SyncPipelineDefinition`; `NewPipelineSpec` → `InlinePipelineConfig` (see #40); `DeltaTableSyncInfo` → `DeltaSyncCheckpoint` (per #35); `ProvisioningInfo` → drop (per #42). Goal: no two types in the package share a generic suffix.
...
### 42. `ProvisioningInfo` empty interface copied verbatim from another proto file — `src/v1/model.ts:570-575`
...
### 43. `ListDatabaseInstanceRolesRequest.pageToken` doc copy-pasta — `src/v1/model.ts:505`
...
### 44. `ListDatabaseCatalogsRequest.pageToken` doc says "synced database tables" — `src/v1/model.ts:491`
- **Why weird:** Same bug: catalogs request says "synced database tables" in doc.
- **Category:** 6 (misleading doc).
- **Suggested name:** Fix to "catalogs".
- **Rationale:** Same as #43.

### 45. `ListDatabaseInstanceRolesResponse.nextPageToken` doc says "next page of instances" — `src/v1/model.ts:514`
- **Why weird:** Doc says "next page of instances" for the roles response.
- **Category:** 6 (misleading doc).
- **Suggested name:** Fix to "roles".
- **Rationale:** Same as #43.

### 46. `CreateDatabaseInstanceRoleRequest.databaseInstanceName` (field) vs `instanceName` (also field) on same request — `src/v1/model.ts:158-160`
...
### 47. `DeleteDatabaseInstanceRoleRequest.reassignOwnedTo` field — `src/v1/model.ts:421`
...
### 48. `DeleteDatabaseInstanceRoleRequest.allowMissing` doc — `src/v1/model.ts:422-423`
...
### 49. `DeleteSyncedDatabaseTableRequest.purgeData` — `src/v1/model.ts:433`
...
### 50. `PACKAGE_SEGMENT` constant — `src/v1/client.ts:78`
...
### 51. `StillRunningError` private error class — `src/v1/client.ts:83`
...
### 52. `client.ts` has a 5-line block-comment at line 633-638 explaining that the role APIs will never reach Public Preview
...
### 53. `findDatabaseInstanceByUid` is the only `findBy*` method
...
### 54. Action-verb conventions in `Client` are consistent
...
- #25 `DatabaseCatalog.databaseProjectId` / `databaseBranchId` / `databaseName` (originally cited at `src/v1/model.ts:219-223`): Fixed in regeneration on 2026-05-20 — `databaseProjectId` and `databaseBranchId` were removed; the remaining `databaseName` / `databaseInstanceName` prefix concern is captured in active finding #23.
