...
### 1. `FindDatabaseInstanceByUidRequest` / `findDatabaseInstanceByUid` — `src/v1/model.ts:497`, `client.ts:437`
- **Why weird:** Verb tense (`Find ... By ...`) is Java/Spring-style. Other clients use `getXById` / `getX` style. The doc on the field at line 498 also says "UID of the **cluster** to get" — referring to a *cluster*, not an instance, contradicting the type name. Same JSDoc bug appears on `GetDatabaseInstanceRequest.name` (model.ts:522): "Name of the **cluster** to get".
...
### 2. `CreateDatabaseInstanceWaiter` exports separately and the wait class is a noun-phrase — `src/v1/client.ts:1022`, `index.ts:3`
...
### 3. `SyncedTableStatus.lastSync: SyncedTablePosition` — `src/v1/model.ts:871`
...
### 4. `DeltaTableSyncInfo` is the only `*SyncInfo` type and the only `Delta*` type — `src/v1/model.ts:487`
...
### 5. `GenerateDatabaseCredentialRequest.claims` (plural `RequestedClaims[]`) but `RequestedClaims` is itself plural — `src/v1/model.ts:514,628`
...
### 6. `GenerateDatabaseCredentialRequest.instanceNames` is "instance_names or claims" — `src/v1/model.ts:505-506`
...
### 7. `NewPipelineSpec` type — `src/v1/model.ts:602`
...
### 8. Repeated generic suffix `*Spec` across the package — `src/v1/model.ts:487,602,774`
...
### 9. `ProvisioningInfo_State` proto-underscore type name — `src/v1/model.ts:161`
- **Why weird:** The enum `ProvisioningInfo_State` carries a proto-style underscore-nested name (`Parent_Nested`). It is referenced by `SyncedDatabaseTable.unityCatalogProvisioningState` (model.ts:683). The underscore-joined name bleeds the proto nesting structure into the public TS surface.
