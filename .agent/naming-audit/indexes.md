...
### 11. `RerankerConfig_RerankerParameters` — underscore-separated type — `src/v1/model.ts:343`
...
- **Rationale:** The underscore costs a lint suppression in two locations and is a leaky proto abstraction. The `RerankerConfig_` prefix is also redundant with the wrapping type — duplicate name (see #12).

### 12. `RerankerConfig.parameters.columnsToRerank` duplicates `QueryVectorIndexRequest.columnsToRerank` — `model.ts:315` vs `model.ts:344`
...
### 13. `effectiveBudgetPolicyId` and `effectiveUsagePolicyId` on `DeltaSyncVectorIndexSpec[Request]` — `model.ts:133-134, 168-169`
...
### 14. `IndexSubtype` enum values include `VECTOR` documented as "Not supported" — `src/v1/model.ts:23-27`
...
### 15. `IndexSubtype` versus `VectorIndexType` — two enums distinguishing two different "type" axes — `model.ts:23, 50`
...
### 16. Enum values are SCREAMING_SNAKE_CASE — `PipelineType.TRIGGERED` etc. — `model.ts:34-37`
...
### 17. `EmbeddingSourceColumn.modelEndpointNameForQuery` — verb-phrase inside a field name — `model.ts:204`
...
### 18. `embeddingSourceColumns` vs `embeddingVectorColumns` — same shape, different role — `model.ts:113-115, 149-150, 181, 189`
...
### 19. `columnsToSync` and `columnsToIndex` — overlapping fields with aliasing — `model.ts:127-141, 161-176`
...
### 20. `pipelineId` is an underspecified ID — `model.ts:123, 158`
...
### 21. `effectiveBudgetPolicyId` on a *request* type without JSDoc explanation — `model.ts:133, 168`
- **Why weird:** See finding #13. Specifically, the field is on `DeltaSyncVectorIndexSpec` (response side) *and* `DeltaSyncVectorIndexSpecRequest` (request side). On the request side, "effective" is incoherent — there is no "effective" until the server resolves it.
...
### 22. `inputsJson` instead of `inputs` — pre-stringified JSON in a JSON request — `model.ts:397`
...
- **Rationale:** Same problem with `filtersJson` (see #23) and `schemaJson` (see #24). All three are wire-protocol leaks that should be normalized at the SDK boundary.

### 23. `filtersJson` instead of `filters` — pre-stringified JSON in a JSON request — `model.ts:305`
- **Why weird:** Same JSON-in-JSON pattern as #22. The JSDoc even shows the JSON structure in examples (`{"id <": 5}`), which means the SDK knows the type — but it is still typed as `string`.
...
### 24. `schemaJson` instead of typed schema — `DirectAccessVectorIndexSpec.schemaJson` — `model.ts:187`
...
### 25. `embeddingWritebackTable` — compound noun reads as gibberish — `model.ts:125, 160`
...
### 26. `ensureRerankerCompatible` boolean — confusing name and confusing semantics — `model.ts:223`
...
### 27. `numResults` field name in two places — `model.ts:291, 367`
...
### 28. `queryType` typed as `string` with constrained values — `QueryVectorIndexRequest.queryType` — `model.ts:313`
...
### 29. `RerankerConfig.model` field — generic name "model" — `model.ts:338`
...
### 30. `Struct.fields` returns `MapStringValueEntry[]` instead of a record — `model.ts:382`
...
### 31. `Value` — single-word generic name for a discriminated union — `model.ts:414-423`
...
### 32. `Struct` — generic single-word type name — `model.ts:380`
...
### 33. `MapStringValueEntry` — verbose proto-style name — `model.ts:245`
...
### 34. `ResultManifest` — Java/Spring-flavored noun — `model.ts:356-361`
...
### 35. `ResultData` — generic two-word noun — `model.ts:348-353`
...
### 36. `lastPrimaryKey` field on request + response — `model.ts:369, 377`
...
### 37. `Call` type imported from `@databricks/sdk-core/api` — generic name — `client.ts:4`
...
### 38. `MiniVectorIndex` is exported from `index.ts` despite being internal-looking — `index.ts:29`
