...
- Types (legacy DBFS): `AddBlock`, `Close`, `Create`, `Delete`, `FileInfo`, `GetStatus`, `ListStatus`, `MkDirs`, `Move`, `Put`, `Read`, plus their response counterparts.
...
- **Category:** 10 (reserved-word/conflict), 6 (misleading).
...
- **Category:** 1 (vague/generic), 6 (misleading shape), 10 (reserved-word collision — `Delete`), 12 (duplicate concept with modern names), 17 (inconsistent verb cluster — Delete/Move/Put are CRUD; AddBlock/Close are stream lifecycle; GetStatus/ListStatus are queries; all in one undifferentiated namespace).
...
- **Rationale:** Carries the surface ("DBFS"), follows the modern `<Action><Resource>Request` shape, no reserved-word collisions, no verb-as-noun ambiguity. Bonus: makes #4 (mixed-surface in single client) much more honest.

### 3. `DBFS` vs `Dbfs` casing — never appears in TS identifier, only in JSDoc — model & client
...
### 4. Package name `files` and class name `Client` are both contextless — `package.json:2`, `client.ts:15,94`
...
### 5. `DirectoryEntry` vs `FileInfo` — duplicate concept inside v2 — `src/v2/model.ts:73,114`
...
- **Why weird:** Both types describe a file-or-directory metadata snapshot, with overlapping fields:
  ...
  - `DirectoryEntry.isDirectory` vs `FileInfo.isDir` (same field, two casings — see #6).
  - `DirectoryEntry.lastModified` vs `FileInfo.modificationTime` (same wire concept, two names — see #7).
...
- **Suggested name:** Pick one. `FileInfo` is the existing standard (also used in `experiments`, `marketplaces` packages — see #8). Re-shape `DirectoryEntry` to extend `FileInfo` with `name`.
- **Rationale:** Two identical-shape types with different field names are a maintenance hazard.

### 6. `isDir` vs `isDirectory` — same concept, two casings — `src/v2/model.ts:77,118,170`
...
### 7. `lastModified` (number-ms-epoch) vs `lastModified` (HTTP-date string) vs `modificationTime` — `src/v2/model.ts:79,110,122,157,174`
...
### 8. `FileInfo` collides with at least two other packages — `src/v2/model.ts:114`
...
- **Suggested name:** `DbfsFileInfo` (or merge with `DirectoryEntry` per #5 — `FileEntry`).
- **Rationale:** `FileInfo` is so generic three different domains felt entitled to use it.

### 9. `Create` returns a `handle` (not the created file) — `src/v2/model.ts:23-34`
...
### 10. `handle: number` — underspecified ID — `src/v2/model.ts:7,17,33`

```ts
AddBlock.handle?: number      // "The handle on an open stream."
Close.handle?:    number
// Response Create handle?: number
```

- **Why weird:** A `number` named `handle` looks like a `numeric ID`, but the JSDoc says it is the result of an `open` (per #9). No type-brand prevents passing arbitrary numbers; no documentation says whether it is positive, monotonic, opaque, or guaranteed unique. The Go SDK uses `int64` here and TS narrows to `number`, which is silently truncated above 2^53 — and there is no mitigation in this client.
...
### 11. `listDirectoryContents` vs `list` — same action, two methods — `src/v2/client.ts:292,656`
...
### 12. `ListDirectoryContentsRequest` paired with `ListDirectoryResponse` — request/response noun mismatch — `src/v2/model.ts:178,206`
...
### 13. `executeCall` / `executeHttpCall` / `sendAndCheckError` / `buildHttpRequest` — `src/v2/utils.ts:26,65,168,96`
...
### 14. `path` as the only field in 5 request types — `src/v2/model.ts:23,45,160,213,224`
...
### 15. `Move.sourcePath` / `Move.destinationPath` use snake_case on the wire — `src/v2/model.ts:484-492`
...
### 16. `contents` field has three different types across model — `src/v2/model.ts:108,246,281,275`

```ts
DownloadFileResponse.contents?: ReadableStream | undefined            // line 108
Put.contents?:                  Uint8Array | undefined                // line 246
// Read response data?:         Uint8Array | undefined  (named 'data')// line 275 — same concept, different name
...
- **Why weird:** "Contents" is overloaded across the model:
  ...
  - Read response `data`: file bytes (chunked read) as a `Uint8Array` — same concept as `Put.contents` but called `data`.
...
### 17. v1 `Client` constructor allows `host` to be undefined — `src/v1/client.ts:21`
...
### 18. v1 `UploadRequest.contents` is `ReadableStream<Uint8Array>` but v2 `UploadFileRequest.contents` is bare `ReadableStream` — `src/v1/model.ts:11`, `src/v2/model.ts:281`
...
### 19. `bytesRead` vs `data` in the read response — singular/plural and naming mismatch — `src/v2/model.ts:267-275`
...
### 20. `ifUnmodifiedSince` — verb-as-noun field — `src/v2/model.ts:101,149`
...
### 21. `range` (HTTP byte range header value) — same — `src/v2/model.ts:95,143`
...
### 22. `directoryPath` vs `path` — inconsistent across methods — `src/v2/model.ts:39,58,128,180`
...
### 23. `recursive` (Delete) — field name does not tell you what it does — `src/v2/model.ts:49`
...
### 25. `nextPageToken` is camelCase but `next_page_token` appears in JSDoc — `src/v2/model.ts:208,210,194,196`
...
### 26. `pageSize` recommendation "We recommend not to set this value..." — `src/v2/model.ts:186`

Doc says don't set the field. Then why is the field public? Name is fine; flagging the inconsistent guidance.

### 27. Read response `data: Uint8Array` is base64 on the wire — `src/v2/model.ts:273`
...
### 28. v2 `index.ts` exports `FileInfo` but `v1/index.ts` does NOT export `DownloadRequest`/`DownloadResponse`-as-used — `src/v1/index.ts:3`
...
### 30. `readAll` is duplicated — `src/v1/utils.ts:23` and `src/v2/utils.ts:40`
...
### 31. `PACKAGE_SEGMENT` — SCREAMING_SNAKE constant — `src/v2/client.ts:89`
...
### 32. `HttpCallOptions` duplicated across files — `src/v1/utils.ts:13`, `src/v2/utils.ts:15`
...
### 33. v1 `encodeFilePath` vs v2 `encodeMultiSegmentPath` — `src/v1/utils.ts:36`, `src/v2/utils.ts:156`
...
### 34. `Move.sourcePath` / `Move.destinationPath` — could be `source` / `destination` — `src/v2/model.ts:233-236`
...
### 35. `pageSize: number` — should mention coercion — `src/v2/model.ts:192`
...
### 36. `pageToken` — opaque token, marked `string | undefined` — `src/v2/model.ts:203`
...
### 37. v1 `Client.upload` returns `Promise<void>` but JSDoc says "does not retry" — `src/v1/client.ts:33-36`
