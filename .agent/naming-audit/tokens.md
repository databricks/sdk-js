...
### 1. `PublicTokenInfo` type name — "public" is unmotivated — `model.ts:56-73`
...
### 2. `publicTokenInfoFieldMask` exported helper — public-API field-mask builder — `model.ts:224`
...
### 3. Method `updateToken` uses URL path interpolation on a potentially empty string — `client.ts:176`
