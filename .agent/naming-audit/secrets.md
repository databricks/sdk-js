...
- **Files / lines:** `src/v1/client.ts:621` (`putAcl`), `:682` (`putSecret`);
  contrast `:138` (`createScope`), `:275` (`deleteSecret`), `:229`
...
- **Issue:** A consumer who learned `createScope` will not guess that the
  ...
  `setSecret`. The JSDoc itself says "Inserts a secret" (client.ts:651) and
  "Creates or overwrites the ACL" (client.ts:588) — three different verbs
...
- **Rationale:** JSDoc on `model.ts:128` says "The associated ACLs rule
...
- **File / line:** `src/v1/model.ts:24` (`ScopeBackendType` enum).
