...
### 4. `replicationPoint: Temporal.Instant` — `src/v1/model.ts:144`
...
### 5. `replicateWorkspaceAssets` field on `WorkspaceSet` — `src/v1/model.ts:320`
...
### 6. `WorkspaceSet.name: string` (resource-name vs human-name ambiguity) — `src/v1/model.ts:309`
...
### 7. `UcCatalog` (and field `catalogs: UcCatalog[]`) — `src/v1/model.ts:278`
...
### 8. `UcReplicationConfig` — `src/v1/model.ts:284`
...
### 10. `WorkspaceSet.stableUrlNames: string[]` (with FQ-name semantics) — `src/v1/model.ts:326`
...
### 11. `dataReplicationWorkspaceSet: string` — `src/v1/model.ts:293`
...
### 12. `etag` field on multiple types — `src/v1/model.ts:78,106,138`
...
### 13. `validateOnly: boolean` — `src/v1/model.ts:44,59`
...
### 14. `parent` field on `Create*Request` / `List*Request` — `src/v1/model.ts:40,55,173,200`
...
### 15. `failoverGroupId` / `stableUrlId` client-provided suffix fields — `src/v1/model.ts:49,64`
...
### 16. `Client` class name — `src/v1/client.ts:52`
...
### 17. `failoverFailoverGroup` method name on `Client` — `src/v1/client.ts:204`
...
### 18. `PACKAGE_SEGMENT` constant — `src/v1/client.ts:47`
...
### 22. Action-verb consistency on `Client` (mostly good)
Methods are `create*`/`get*`/`list*`/`update*`/`delete*` plus one bespoke action (`failoverFailoverGroup`). Aside from the stutter (#17), this is consistent. Listed as observation per rule 17 since the audit asks to flag inconsistencies — here only the one method breaks the pattern.

### 23. Acronym casing inconsistency: `URL` vs `Uri` vs `Url`
Within this package:
...
- **UC** — Unity Catalog. Appears as the `Uc` prefix on `UcCatalog`, `UcReplicationConfig`, and as `unityCatalog…` in field names (inconsistency, see #8).
