...
### 3. `EntityTagAssignment` field shape vs. sister `TagAssignment` shape — `src/v1/model.ts:32-47` vs. `tagassignments/src/v1/model.ts:46-55`
...
### 5. `entityName: string` doc says "fully qualified name" but type does not enforce — `src/v1/model.ts:24,34,52,62`
...
### 6. `entityType: string` everywhere — `src/v1/model.ts:28,40,56,68`
...
### 7. `tagKey` field doc inconsistency: required marker on get/delete, not on `EntityTagAssignment` — `src/v1/model.ts:26,36,54`
...
### 8. `Client` class — `src/v1/client.ts:41`
...
### 9. Method names `createEntityTagAssignment` / `deleteEntityTagAssignment` / `getEntityTagAssignment` / `listEntityTagAssignments` / `updateEntityTagAssignment` — `src/v1/client.ts:76,114,133,163,226`
...
### 10. `CreateEntityTagAssignmentRequest` etc. — five request DTOs share a 25-char prefix — `src/v1/model.ts:17,22,50,60,79`
...
### 11. `ListEntityTagAssignmentsRequest` (plural) vs. `EntityTagAssignment` (singular) — `src/v1/model.ts:60` vs. `src/v1/model.ts:32`
...
### 13. `Call` type and `call` variable — `src/v1/client.ts:86,119,139,178,242` and `src/v1/utils.ts:27`
...
### 14. `req.entityType ?? ''` / `req.entityName ?? ''` / `req.tagKey ?? ''` URL composition — `src/v1/client.ts:118,137,167,230`
...
### 15. `respBody` (raw bytes) vs. `resp` (parsed object) — `src/v1/client.ts:90-95, 143-148, 182-187, 252-257`
...
### 16. `httpReq` local variable — `src/v1/client.ts:89,122,142,181,245`
...
### 18. `buildHttpRequest` returns `HttpRequest` — `src/v1/utils.ts:96`
...
### 20. `readAll(body)` — `src/v1/utils.ts:40`
...
### 21. `PACKAGE_SEGMENT` — `src/v1/client.ts:36`
...
### 22. `tagKey` and `tagValue` co-located on `EntityTagAssignment` — `src/v1/model.ts:36,38`
...
### 23. `updateTime` and `updatedBy` paired field naming — `src/v1/model.ts:42,44`
...
### 26. `entitytagassignments` lowercase package name
...
### 27. Domain leakage from sister packages
