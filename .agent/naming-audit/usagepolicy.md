...
### 4. `UsagePolicy.bindingWorkspaceIds: number[]` representation — `src/v1/model.ts:129`
...
### 5. `CustomPolicyTag` type name — `src/v1/model.ts:23`
...
### 6. `CustomPolicyTag` reserved-key documentation refers to `budget-policy-*` keys — `src/v1/model.ts:27-29`
...
### 7. `Filter.creatorUserId: number` representation — `src/v1/model.ts:57`
...
- **Rationale:** Same as finding #4 but for the filter field. Generator-wide concern; same problem on `bindingWorkspaceIds: number[]`.

### 8. `SortSpec` type — `src/v1/model.ts:103`
...
### 9. `SortSpec.field` JSDoc typo "The filed to sort by" — `src/v1/model.ts:104`
...
### 10. `ListUsagePoliciesResponse.previousPageToken` — `src/v1/model.ts:100`
...
### 11. `CreateUsagePolicyRequest.policy` field with wire-name leak in JSDoc — `src/v1/model.ts:19-20`
...
### 12. `UpdateUsagePolicyRequest.policy` doc mentions `creator_user_id` — `src/v1/model.ts:112`
...
### 13. `UpdateUsagePolicyRequest` has **no** `updateMask` field — `src/v1/model.ts:111-118`
...
### 14. `executeCall` / `executeHttpCall` naming pair — `src/v1/utils.ts:26,65`
...
### 15. `HttpCallOptions` — `src/v1/utils.ts:15`
...
### 18. `PACKAGE_SEGMENT` constant — `src/v1/client.ts:41`
...
### 19. `Client` class plain name — `src/v1/client.ts:46`
...
### 20. `req` parameter name on client methods — `src/v1/client.ts:77,103,122,147,194,212`
...
### 21. JSDoc on `getUsagePolicy` reads "Retrieves a usage policy by it's ID." — `src/v1/client.ts:120`
...
### 22. `SortSpec_Field` proto-style underscore-nested enum name — `src/v1/model.ts:6`
...
### 23. `buildHttpRequest` — `Http` mid-position infix — `src/v1/utils.ts:96`
...
### 24. URL-path version split (`/api/2.1` vs `/api/2.0`) is the only meaningful API surface difference
...
### 25. No `FieldMask` import in `usagepolicy/src/v1/model.ts`
...
### 27. Acronym casing `Id` consistently used as `Id`, not `ID`
...
### 28. Wire-form vs kebab-case vs TS casings (`usage_policies` / `usage-policies` / `usagePolicies`)
...
- `custom tag` — A `{key, value}` pair attached to billing usage. Reserved keys per JSDoc: `budget-policy-name`, `budget-policy-id`, `budget-policy-resolution-result` (note: keys retain the `budget-policy-` prefix even though this is the *usage* policy package — see finding #6).
