...
### 6. `ColumnRelationship` ambiguous source/target — `src/v1/model.ts:42-45`
...
### 7. `Client` class — `src/v1/client.ts:45`
...
### 9. `buildHttpRequest` returns `HttpRequest` — `src/v1/utils.ts:96`
...
### 10. `HttpCallOptions` — `src/v1/utils.ts:15`
...
### 12. `LineageTableInfo.name` — `src/v1/model.ts:201`
...
### 13. `ExternalLineageRelationshipExternalMetadata.name` — `src/v1/model.ts:131`
...
### 14. `ExternalLineageRelationshipPath.url` — `src/v1/model.ts:155`
...
### 15. `LineageModelVersionInfo.modelName` vs `version` — `src/v1/model.ts:191, 193`
...
### 16. `LineageFileInfo.securableName`, `securableType`, `storageLocation` — `src/v1/model.ts:179-183`
...
### 17. `eventTime` repeated on four sibling types — `src/v1/model.ts:171, 185, 195, 207`
...
### 18. `req` parameter and `respBody` / `resp` locals — `src/v1/client.ts:72, 80-92, 104, 134-178, 202-235`
...
### 19. `httpReq` local variable — `src/v1/client.ts:84, 123, 162, 220`
...
### 20. `Call` type and `call` variable — `src/v1/client.ts:81, 120, 159, 217`
...
### 21. `PACKAGE_SEGMENT` — `src/v1/client.ts:40`
...
### 22. `ListExternalLineageRelationshipsResponse.externalLineageRelationships` field name
...
- `Model Version` — MLflow registered-model version, identified by `(modelName, version)` pair. Note the type-drift between `number` (in `LineageModelVersionInfo`) and `string` (in `ExternalLineageRelationshipModelVersion`) — see #15.
