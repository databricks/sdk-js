...
### 1. `SystemType.SYSTEM_TYPE_UNSPECIFIED` and 22 other SCREAMING_SNAKE values — `src/v1/model.ts:8-33`
- **Why weird:** Every enum value is SCREAMING_SNAKE_CASE and the sentinel re-states the enum name (`SystemType.SYSTEM_TYPE_UNSPECIFIED`). The remaining 22 values include long product names (`MICROSOFT_SQL_SERVER`, `AMAZON_REDSHIFT`, `GOOGLE_BIGQUERY`, `MICROSOFT_FABRIC`) that are sometimes split on underscores in a way that conflicts with how the rest of the SDK casefolds acronyms (e.g., `POWER_BI` vs. `POWERBI`).
- **Category:** 2 (redundant enum prefix on `SYSTEM_TYPE_UNSPECIFIED`), 3 (acronym casing — `POWER_BI` vs `MICROSOFT_FABRIC`), 18 (long enum values).
- **Suggested name:** `SystemType.Tableau`, `SystemType.PowerBI`, `SystemType.SqlServer`, `SystemType.Redshift`, etc., dropping `Unspecified`.
- **Rationale:** TS enum values are namespaced by the enum (`SystemType.Tableau`). SCREAMING_SNAKE_CASE forces callers to shout every value; idiomatic TS uses PascalCase enum members (see TypeScript handbook on enums and the Google TypeScript style guide).

### 2. `CreateRequestExternalLineage` / `DeleteRequestExternalLineage` / `UpdateRequestExternalLineage` — `src/v1/model.ts:51, 74, 238`
...
### 3. `ExternalLineageRelationshipObject.tpe` discriminator — `src/v1/model.ts:140`
...
### 4. `objectInfo` field of type `ExternalLineageRelationshipObject` — `src/v1/model.ts:216`
...
### 5. `ExternalLineageInfo` vs. `ExternalLineageRelationship` — `src/v1/model.ts:98, 111`
...
### 6. Mixed `Info` / `Relationship` / `Object` suffix vocabulary — across `src/v1/model.ts`
...
### 7. `ExternalLineageRelationshipTable` / `ExternalLineageRelationshipPath` / `ExternalLineageRelationshipModelVersion` / `ExternalLineageRelationshipExternalMetadata` — `src/v1/model.ts:158, 154, 134, 130`
...
### 8. `ColumnRelationship` ambiguous source/target — `src/v1/model.ts:42-45`
...
### 9. `Client` class — `src/v1/client.ts:45`
...
### 10. `executeCall` vs. `executeHttpCall` — `src/v1/utils.ts:26, 65`
...
### 11. `buildHttpRequest` returns `HttpRequest` — `src/v1/utils.ts:96`
...
### 14. `LineageTableInfo.name` — `src/v1/model.ts:201`
...
### 15. `ExternalLineageRelationshipExternalMetadata.name` — `src/v1/model.ts:131`
...
### 16. `ExternalLineageRelationshipPath.url` — `src/v1/model.ts:155`
...
### 17. `LineageModelVersionInfo.modelName` vs `version` — `src/v1/model.ts:191, 193`
- **Why weird:** Type carries `modelName` (string) and `version` (number). The `version` is described as "Version number of the model" — but the related type `ExternalLineageRelationshipModelVersion` (#7) uses `version: string`. Same concept, two types: `number` in the response, `string` on the relationship side.
...
### 18. `LineageFileInfo.securableName`, `securableType`, `storageLocation` — `src/v1/model.ts:179-183`
...
### 19. `eventTime` repeated on four sibling types — `src/v1/model.ts:171, 185, 195, 207`
...
### 20. `req` parameter and `respBody` / `resp` locals — `src/v1/client.ts:72, 85-92, 107, 134-186, 202-235`
...
### 21. `httpReq` local variable — `src/v1/client.ts:84, 123, 162, 220`
...
### 22. `Call` type and `call` variable — `src/v1/client.ts:81, 120, 159, 217`
...
### 23. `PACKAGE_SEGMENT` — `src/v1/client.ts:40`
...
### 24. Method names re-state the entity verbosely
...
- `Model Version` — MLflow registered-model version, identified by `(modelName, version)` pair. Note the type-drift between `number` (in `LineageModelVersionInfo`) and `string` (in `ExternalLineageRelationshipModelVersion`) — see #17.
