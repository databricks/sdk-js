...
### 1. `Disposition` enum lacks SDK context — `src/v1/model.ts:40`
...
### 2. `Format` enum is dangerously generic — `src/v1/model.ts:46`
...
### 3. `TimeoutAction` enum name doesn't tie to the timeout field — `src/v1/model.ts:77`
...
### 4. `getStatementResult` method conflates "poll" with "fetch result" — `src/v1/client.ts:230`
...
### 5. `getResultData` method drops resource and chunk-index semantics — `src/v1/client.ts:191`
...
### 6. `warehouseId` field — `src/v1/model.ts:158`
...
### 7. `chunkIndex` field type is `number` in URL path — `src/v1/client.ts:195`
...
### 8. `truncated` field on `ResultManifest` is ambiguously boolean — `src/v1/model.ts:464`
...
### 9. `dataArray` JSDoc references `JSON_ARRAY` format — `src/v1/model.ts:422`
...
### 10. `externalLinks?: ExternalLink[]` + `dataArray?: JsonValue[][]` mutual exclusivity — `src/v1/model.ts:417, 422`
...
### 12. `ServiceError` — `Service` is architectural-layer leak — `src/v1/model.ts:473`
...
### 13. `ServiceErrorCode` — `Service` is architectural-layer leak — `src/v1/model.ts:53`
...
### 14. `StatementStatus_State` — proto-style nested-type underscore — `src/v1/model.ts:84`
...
### 15. `ExternalLink_HttpHeadersEntry` — proto map-entry leak — `src/v1/model.ts:376`
...
### 16. `*_UNSPECIFIED` enum zero values — `src/v1/model.ts:41, 47, 78, 85`
...
### 17. `chunkIndex` vs `nextChunkIndex` naming pair — `src/v1/model.ts:107, 122`
...
### 18. `httpHeaders` value type loses sensitivity context — `src/v1/model.ts:349`
...
### 19. `Schema` type is a top-level export with a maximally overloaded name — `src/v1/model.ts:468`
...
### O-1. URL string concatenation handles `undefined` silently — `src/v1/client.ts:78, 195, 234`
