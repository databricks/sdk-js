...
### 1. `WorkspaceBaseEnvironment` — type name is a 26-character three-adjective noun phrase — `model.ts:718`
...
### 2. `WorkspaceBaseEnvironment.status` field type's name and the type's domain do not align — `model.ts:737`
