...
### 1. `Environment` concept fragmented across `environments` and `clusterlibraries` packages — `packages/environments/` vs `packages/clusterlibraries/`
...
### 2. `WorkspaceBaseEnvironment` — type name is a 26-character three-adjective noun phrase — `model.ts:718`
...
### 3. `WorkspaceBaseEnvironment.status` field name and type's domain do not align — `model.ts:737`
...
### 4. `Operation` exported with no namespace prefix — collides on a single-import surface — `model.ts:641, index.ts:26`
...
### 5. `Operation.result` discriminated union uses `$case: 'error' | 'response'` — naming verbose & inconsistent — `model.ts:666-677`
...
### 6. `DatabricksServiceExceptionWithDetailsProto` — Proto suffix on a TS type, "Exception" implies an Error class — `model.ts:552, index.ts:18`
...
### 7. `ErrorCode` enum exports ~100 values and is mostly deprecated — `model.ts:16-514, index.ts:12`
...
### 8. `ErrorCode` values are SCREAMING_SNAKE strings, e.g. `'PROVIDER_SHARE_NOT_ACCESSIBLE'` — `model.ts:513`
...
### 9. `DatabricksServiceExceptionWithDetailsProto` — `Service` mid-position is an architectural-layer leak, not domain — `model.ts:552, index.ts:18`
...
### 10. `WorkspaceBaseEnvironment.isDefault` — boolean field on the resource, but `DefaultWorkspaceBaseEnvironment` is a separate type — `model.ts:741`
