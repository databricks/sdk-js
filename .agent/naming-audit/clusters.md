...
### 1. `clusterLogStatus` field typed `LogSyncStatus` — `src/v2/model.ts:1392`
...
### 2. `ClusterEventType_ClusterEventType` doubly-nested enum name — `src/v2/model.ts:832`
...
### 3. `DataPlaneEventDetails` / `DataPlaneClusterEventType` — control-plane vs data-plane infrastructure naming — `src/v2/model.ts:2048,95`
...
### 4. `AutoScale` type name — `src/v2/model.ts:1021`
