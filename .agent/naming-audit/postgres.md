...
### 1. `postgres` and `database` packages overlap heavily — `packages/postgres/` vs `packages/database/`
...
### 2. `ErrorCode` enum — 102 long, mostly-deprecated values — `src/v1/model.ts:17-515`
...
### 3. Three "State" enums share value vocabulary but use two different qualifier patterns — `src/v1/model.ts:581, 600, 610`
...
### 4. `Role_Attributes.createdb` / `createrole` / `bypassrls` — Postgres-keyword-style lowercase fields — `src/v1/model.ts:1793-1795`
...
### 5. `Role_AuthMethod.PG_PASSWORD_SCRAM_SHA_256` and `LAKEBASE_OAUTH_V1` enum values — `src/v1/model.ts:633, 638`
...
### 6. `DatabricksServiceExceptionWithDetailsProto.details: Record<string, unknown>[]` — array of opaque records — `src/v1/model.ts:1094`
...
### 7. 21 separate `*Operation` classes — one per CRUD verb per resource — `src/v1/client.ts:1512-3345`
...
### 8. `DatabricksServiceExceptionWithDetailsProto` — proto-architectural-leak type name — `src/v1/model.ts:1090`, `index.ts:64`
...
### 9. `Branch` / `Catalog` / `Database` / `Endpoint` / `Project` / `Role` / `SyncedTable` — 7 generic top-level resource names — `src/v1/model.ts` (multiple)
...
### 10. `BranchSpec.expiration` discriminated union — `noExpiry: boolean` accepts an invalid `false` — `src/v1/model.ts:736-762`
...
### 11. `Database` (SDK resource) vs `postgresDatabase` field (Postgres-side identifier) — same thing, two names — `src/v1/model.ts:1013, 1054`
...
### 12. `Database` and `databaseId` query parameter for `createDatabase` — `src/v1/client.ts:245-289`, `model.ts:939`
...
### 13. `EndpointSpec.autoscalingLimitMinCu` / `autoscalingLimitMaxCu` — `Cu` suffix is opaque — `src/v1/model.ts:1274, 1279`
...
### 14. `EndpointSpec.suspension` discriminated union — `noSuspension: boolean` accepts an invalid `false` — `src/v1/model.ts:1293-1312`
- **Why weird:** Same pattern as #10 — one variant carries a duration, the other a boolean documented as accepting only `true`. The type permits `false`, the spec rejects it.
- **Category:** 16 (type allows `false` but spec rejects), 27 (echo of #10).
- **Suggested name:** Inline: `suspension?: Temporal.Duration | 'never'`.
- **Rationale:** Same as #10.

### 15. `GenerateDatabaseCredentialRequest.expiration` discriminated union — _removed in regeneration_; only the simpler `claims` + `endpoint` shape remains — see #23
_Reserved._

### 16. `Operation.metadata: Record<string, unknown>` — opaque metadata field — `src/v1/model.ts:1575`
...
- **Rationale:** Same root cause as #6 — opaque records on the public surface.

### 17. `Operation.result` discriminated union with `error` / `response` — `src/v1/model.ts:1588-1599`
...
- **Suggested name:** Same as #16 — generic `Operation<TResult, TMetadata>` with both arms typed.
- **Rationale:** Same as #6, #16.

### 18. `Project.initialEndpointSpec` — write-only field exposed on read shape — `src/v1/model.ts:1624`
...
### 19. `ProjectCustomTag` vs the `database` package's `CustomTag` — `src/v1/model.ts:1637`, `database:206`
...
### 20. `ProjectSpec.pgVersion: number` vs `ProjectStatus.pgVersion: number` — Postgres version as integer — `src/v1/model.ts:1687, 1716`
...
### 21. `ProjectSpec.historyRetentionDuration` vs `ProjectStatus.historyRetentionDuration` — copy of input on output — `src/v1/model.ts:1689, 1718`
...
### 22. `timeseriesKey` field casing on the synced-table spec — `src/v1/model.ts:1936`
...
### 23. Synced-table spec fields `createDatabaseObjectsIfMissing` — same fields, same issues as `database` package — `src/v1/model.ts:1951`
...
### 24. `UpdateBranchRequest.updateMask: FieldMask<Branch>` — Google API protocol leak — `src/v1/model.ts:2057`
...
### 25. `CreateBranchRequest.replaceExisting` (Create) — no symmetrical `allowMissing` on Delete (Delete uses `purge`) — `src/v1/model.ts:911, 1104, 1142`
...
### 26. `DeleteBranchRequest.purge` — boolean for hard delete — `src/v1/model.ts:1104`
...
### 27. `GenerateDatabaseCredentialRequest.claims: RequestedClaims[]` — plural of a plural type — `src/v1/model.ts:1363`
...
### 28. `Operation.done: boolean | undefined` — tri-state boolean — `src/v1/model.ts:1581`
...
### 29. `Operation` is a separate type, not a generic — `src/v1/model.ts:1563`
- **Why weird:** All 21 mutation methods return `Promise<Operation>`. The `Operation` type is monomorphic — no generic parameter for result/metadata. Consumer either uses the per-resource `*Operation` waiter classes (#7) or reads `Operation.result.response` (untyped `Record`).
- **Category:** Observation (architecture, not naming per se).
- **Suggested name:** `Operation<TResult, TMetadata>` generic.
- **Rationale:** Connects #6, #7, #16, #17.
