...
### 12. `QueryStatementType` — verbose discriminator — `src/v1/model.ts:29`
...
### 13. `QueryInfo.executionEndTimeMs` vs `queryEndTimeMs` — domain confusion — `src/v1/model.ts:195,197`
...
### 14. `QueryInfo.lookupKey` — cryptic, undocumented — `src/v1/model.ts:211`
...
### 15. `QueryInfo.executedAsUserId` / `executedAsUserName` — duplicate of `userId` / `userName` — `src/v1/model.ts:215,217`
...
### 16. `QueryInfo.userName` is "email or username" — `src/v1/model.ts:201`
...
### 17. `QueryInfo.sparkUiUrl` — implementation leak — `src/v1/model.ts:203`
...
### 18. `QueryInfo.warehouseId` and `endpointId` co-existing — `src/v1/model.ts:205,232`
...
### 19. `QueryInfo.sessionId` — overloaded identifier — `src/v1/model.ts:222`
...
### 20. `QueryInfo.isFinal` — what does "final" mean? — `src/v1/model.ts:224`
...
### 21. `QueryInfo.channelUsed` vs type `ChannelInfo` — `src/v1/model.ts:226`
...
### 22. `QueryInfo.metrics: QueryMetrics` vs `QueryInfo.duration: number` — `src/v1/model.ts:213,237`
...
### 23. `QueryInfo.duration` — unit not in the name — `src/v1/model.ts:237`
...
- **Suggested name:** `durationMs`. (See also #22.)
- **Rationale:** Internal consistency with every other time field in the file.

## Low severity

### 24. `QueryInfo.clientApplication` — domain ambiguity — `src/v1/model.ts:243`
...
### 25. `QueryInfo.querySource: ExternalQuerySource` vs `cacheQueryId` — `src/v1/model.ts:248,250`
...
### 26. `ExternalQuerySource.dashboardId` vs `legacyDashboardId` — `src/v1/model.ts:99,101`
...
### 27. `ExternalQuerySource.alertId` and `sqlQueryId` and `genieSpaceId` — — `src/v1/model.ts:103,107,110`
...
### 28. `ExternalQuerySource_JobInfo.jobTaskRunId` — — `src/v1/model.ts:120`
...
### 29. `QueryMetrics.totalTimeMs` vs `executionTimeMs` vs `taskTotalTimeMs` vs `photonTotalTimeMs` — — `src/v1/model.ts:261,269,279,285`
...
### 30. `QueryMetrics.workToBeDone` — phrase as field name — `src/v1/model.ts:319`
...
### 31. `QueryMetrics.runnableTasks` — — `src/v1/model.ts:324`
...
### 32. `QueryMetrics.projectedRemainingTaskTotalTimeMs` and `projectedRemainingWallclockTimeMs` — — `src/v1/model.ts:326,333`
...
### 33. `QueryMetrics.spillToDiskBytes` / `readRemoteBytes` / `writeRemoteBytes` / `readCacheBytes` / `networkSentBytes` / `readFilesBytes` / `prunedBytes` — — `src/v1/model.ts:271,273,275,277,291,295,335`
...
### 34. `QueryMetrics.readBytes` vs `readFilesBytes` — — `src/v1/model.ts:263,335`
...
### 35. `QueryMetrics.prunedBytes` / `prunedFilesCount` paired with `readFilesBytes` / `readFilesCount` — — `src/v1/model.ts:281,295,297,335`
...
### 36. `QueryMetrics.rowsProducedCount` vs `QueryInfo.rowsProduced` — — `src/v1/model.ts:265,207`
...
### 37. `QueryMetrics.provisioningQueueStartTimestamp` / `overloadingQueueStartTimestamp` / `queryCompilationStartTimestamp` — `Timestamp` suffix inconsistency — `src/v1/model.ts:302,307,309`
...
### 38. `QueryMetrics.taskTimeOverTimeRange: TaskTimeOverRange` — — `src/v1/model.ts:314`
...
### 39. `TaskTimeOverRange.entries` / `TaskTimeOverRangeEntry` — — `src/v1/model.ts:349,358`
...
### 40. `TaskTimeOverRangeEntry.taskCompletedTimeMs` — — `src/v1/model.ts:360`
...
### 41. `TimeRange.startTimeMs` / `endTimeMs` — — `src/v1/model.ts:365,367`
...
### 42. `QueryFilter.statuses` doc — recommends against using it — `src/v1/model.ts:170`
...
### 43. `QueryTag.key` / `QueryTag.value` — both optional — `src/v1/model.ts:344,345`
...
### 44. `ExternalQuerySource.legacyDashboardId` — `Legacy` mid-position architectural-leak modifier — `src/v1/model.ts:101`
...
- **Suggested name:** `redashDashboardId` (or whatever the underlying product is actually called), or fold into a discriminated union as suggested in #26.
