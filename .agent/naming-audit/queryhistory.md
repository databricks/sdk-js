...
- **Why weird:** Top-level interface called `ListQueries`. Verb-as-noun: `ListQueries` is grammatically a command (the *action*), not a *thing*. Every other Databricks SDK package uses `List<Plural>Request` (e.g. `ListAlertsRequest`, `ListJobsRequest`) for the request body. The mismatch shows up at the call site:
  ...
  ```
  reads as "list queries, given a thing called listQueries", which is circular.
- **Category:** 13, 14, 17 (verb-tense inconsistency; Go/Java-style name; inconsistent action verbs)
- **Suggested name:** `ListQueriesRequest`.
- **Rationale:** Aligns with every other package in the SDK and removes the verb-as-noun footgun.

### 2. `QueryInfo` — "Info" suffix on a top-level domain entity — `src/v1/model.ts:179`
...
- **Rationale:** A list of queries should be `Query[]`, not `QueryInfo[]`. The field is even called `res?: QueryInfo[]` (see #3) — `res?: Query[]` reads better. The Go SDK uses `QueryInfo` because Go does not have a `Query` type collision in this package; TS does not have that constraint and should pick the cleaner name.

### 3. `ListQueries_Response.res` — cryptic field — `src/v1/model.ts:158`
...
- **Rationale:** The field is `QueryInfo[]` (or `Query[]` after #2). `queries` is the obvious idiomatic name. Even keeping the wire form `res`, the TS surface can map `res` → `queries` in the unmarshal transform (the file already does property-renames everywhere — `query_id` → `queryId`, etc.).

### 4. `QueryInfo.endpointId` aliased to `warehouse_id` — `src/v1/model.ts:207`
...
### 5. `CHANNEL_NAME_*` redundant enum prefix — `src/v1/model.ts:5-11`
...
### 6. `ChannelName.CHANNEL_NAME_UNSPECIFIED` — proto-style sentinel exposed — `src/v1/model.ts:6`
...
### 7. `PlansState` — vague type name — `src/v1/model.ts:14`
...
### 8. `PlansState.EXISTS` vs `EMPTY` — verb-tense inconsistency — `src/v1/model.ts:20,24`
...
### 9. `PlansState.IGNORED_LARGE_PLANS_SIZE` — overly verbose and grammatically off — `src/v1/model.ts:18`
...
### 10. `PlansState.IGNORED_SPARK_PLAN_TYPE` — domain-leaky enum value — `src/v1/model.ts:26`
...
### 11. `QueryStatementType.OTHER` and `CALL` — `src/v1/model.ts:30,53`
...
### 12. `QueryStatus.CANCELED` — spelling and verb-tense — `src/v1/model.ts:84`
...
### 13. `QueryStatus.STARTED` and `COMPILED` — deprecated but exported — `src/v1/model.ts:67,77`
...
### 14. `QueryStatementType` — verbose discriminator — `src/v1/model.ts:29`
...
### 15. `QueryInfo.executionEndTimeMs` vs `queryEndTimeMs` — domain confusion — `src/v1/model.ts:197,199`
...
### 16. `QueryInfo.lookupKey` — cryptic, undocumented — `src/v1/model.ts:213`
...
### 17. `QueryInfo.executedAsUserId` / `executedAsUserName` — duplicate of `userId` / `userName` — `src/v1/model.ts:217,219`
...
### 18. `QueryInfo.userName` is "email or username" — `src/v1/model.ts:203`
...
### 19. `QueryInfo.sparkUiUrl` — implementation leak — `src/v1/model.ts:205`
...
### 20. `QueryInfo.warehouseId` and `endpointId` co-existing — `src/v1/model.ts:207,234`
- **Why weird:** Cross-reference of #4: these two fields both exist on `QueryInfo`. The audit calls out the *duplication*; the names *individually* are also weak — `warehouseId` is fine; `endpointId` is misleading (the wire form keeps it for back-compat with the old SQL Endpoint API).
- **Category:** 19, 16 (underspecified ID; field contradicting type domain)
- **Suggested name:** See #4.

### 21. `QueryInfo.sessionId` — overloaded identifier — `src/v1/model.ts:224`
...
### 22. `QueryInfo.isFinal` — what does "final" mean? — `src/v1/model.ts:226`
...
### 23. `QueryInfo.channelUsed` vs type `ChannelInfo` — `src/v1/model.ts:228`
- **Why weird:** Field `channelUsed: ChannelInfo`. The `Used` suffix is unusual (past participle on a noun). Most fields elsewhere drop the verb: `channel`, `warehouse`, etc. The type itself is `ChannelInfo` (another `Info` suffix — see #2 / general pattern). Reads as "channel-used info" — three nouns.
...
### 24. `QueryInfo.metrics: QueryMetrics` vs `QueryInfo.duration: number` — `src/v1/model.ts:215,239`
...
### 25. `QueryInfo.duration` — unit not in the name — `src/v1/model.ts:239`
...
- **Suggested name:** `durationMs`. (See also #24.)
- **Rationale:** Internal consistency with every other time field in the file.

## Low severity

### 26. `QueryInfo.clientApplication` — domain ambiguity — `src/v1/model.ts:245`
...
### 27. `QueryInfo.querySource: ExternalQuerySource` vs `cacheQueryId` — `src/v1/model.ts:250,252`
...
### 28. `ExternalQuerySource.dashboardId` vs `legacyDashboardId` — `src/v1/model.ts:101,103`
...
### 29. `ExternalQuerySource.alertId` and `sqlQueryId` and `genieSpaceId` — — `src/v1/model.ts:105,109,112`
...
### 30. `ExternalQuerySource_JobInfo.jobTaskRunId` — — `src/v1/model.ts:122`
...
### 31. `QueryMetrics.totalTimeMs` vs `executionTimeMs` vs `taskTotalTimeMs` vs `photonTotalTimeMs` — — `src/v1/model.ts:263,271,281,287`
...
### 32. `QueryMetrics.workToBeDone` — phrase as field name — `src/v1/model.ts:321`
...
### 33. `QueryMetrics.runnableTasks` — — `src/v1/model.ts:326`
...
### 34. `QueryMetrics.projectedRemainingTaskTotalTimeMs` and `projectedRemainingWallclockTimeMs` — — `src/v1/model.ts:328,335`
...
### 35. `QueryMetrics.spillToDiskBytes` / `readRemoteBytes` / `writeRemoteBytes` / `readCacheBytes` / `networkSentBytes` / `readFilesBytes` / `prunedBytes` — — `src/v1/model.ts:273,275,277,279,293,297,337`
...
### 36. `QueryMetrics.readBytes` vs `readFilesBytes` — — `src/v1/model.ts:265,337`
...
### 37. `QueryMetrics.prunedBytes` / `prunedFilesCount` paired with `readFilesBytes` / `readFilesCount` — — `src/v1/model.ts:283,297,299,337`
...
### 38. `QueryMetrics.rowsProducedCount` vs `QueryInfo.rowsProduced` — — `src/v1/model.ts:267, 209`
...
### 39. `QueryMetrics.provisioningQueueStartTimestamp` / `overloadingQueueStartTimestamp` / `queryCompilationStartTimestamp` — `Timestamp` suffix inconsistency — `src/v1/model.ts:304,309,311`
...
### 40. `QueryMetrics.taskTimeOverTimeRange: TaskTimeOverRange` — — `src/v1/model.ts:316`
...
### 41. `TaskTimeOverRange.entries` / `TaskTimeOverRangeEntry` — — `src/v1/model.ts:351,360`
...
### 42. `TaskTimeOverRangeEntry.taskCompletedTimeMs` — — `src/v1/model.ts:362`
...
### 43. `TimeRange.startTimeMs` / `endTimeMs` — — `src/v1/model.ts:367,369`
...
### 44. `QueryFilter.statuses` doc — recommends against using it — `src/v1/model.ts:172`
...
### 45. `QueryTag.key` / `QueryTag.value` — both optional — `src/v1/model.ts:346,347`
...
2. **Redundant enum-value prefixes and proto sentinels.** `CHANNEL_NAME_*` doubles the enum name in every value; `UNSPECIFIED` exposes the proto3 zero-value sentinel into the TS surface where `undefined` already serves.
