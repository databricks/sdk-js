...
### 6. `ListTagPoliciesRequest` (plural) vs. `TagPolicy` (singular) — `src/v1/model.ts:20` vs. `model.ts:36`
...
### 7. Create/update wrap the subject in a body field while the list response also wraps in an array field — `src/v1/model.ts:9,32,48`
...
### 8. `executeCall` vs. `executeHttpCall` confusion — `src/v1/utils.ts:26,65`
...
### 10. `Temporal.Instant` for timestamps — `src/v1/model.ts:42,44`
...
### 11. `pageSize` upper-bound documented in doc, not enforced in type — `src/v1/model.ts:22-25`
...
### 12. `updateMask` field type `FieldMask<TagPolicy>` — `src/v1/model.ts:49`
...
### 15. `tagpolicies` lowercase package name
...
### 16. Domain leakage from sister packages
