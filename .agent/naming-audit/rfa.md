...
### 3. `Securable.type` field collides with reserved word & loses meaning — `model.ts:160`
...
### 4. `Securable.fullName` doc says "catalog/schema/table" but reality is broader — `model.ts:162-165`
...
### 5. `AccessRequestDestinations.securableType` and `fullName` duplicate `securable.type` and `securable.fullName` — `model.ts:54-73`
...
### 6. `GetAccessRequestDestinationsRequest.securableType` typed as `string` — `model.ts:121-126`
...
### 7. Type name `AccessRequestDestinations` is plural — but represents ONE securable's destinations — `model.ts:54-73`
...
### 8. `BatchCreateAccessRequestsRequest` / `BatchCreateAccessRequestsResponse` — `model.ts:75,85`
...
### 9. `BatchCreateAccessRequestsResponse.responses` — `model.ts:85-88`
...
### 10. `CreateAccessRequest.behalfOf` field — `model.ts:90-97`
...
### 11. `CreateAccessRequest.securablePermissions` is array but bare `securable` siblings are singular — `model.ts:111`
...
### 12. `CreateAccessRequestResponse.requestDestinations` — `model.ts:114-119`
...
### 13. `NotificationDestination.destinationId` — `model.ts:128-134`
...
### 14. `NotificationDestination.destinationType` — `model.ts:135`
...
- **Rationale:** Wire stays `destination_type`; TS can drop the prefix the same way `Securable.type` does (model.ts:160) — note the inconsistency within the same project.

### 15. `NotificationDestination.specialDestination` overloads with `destinationType` — `model.ts:136-142`
...
### 16. `Principal.id` is bare `id` — but holds either user, group, or service principal ID — `model.ts:145-149`
...
### 17. `Securable.providerShare` — `model.ts:166-170`
...
### 18. `SecurableType.STAGING_TABLE` and inline TODO — `model.ts:41-42`
...
### 19. `SecurableType.EXTERNAL_METADATA` lacks doc — `model.ts:40`
...
### 20. `Principal` is exported but `principalType` field has no doc — `model.ts:148`
...
### 21. `SecurablePermissions.permissions: string[]` — `model.ts:173-178`
...
- **Rationale:** Same problem as #6. The wire is string, but TS could narrow it.

### 22. Method `batchCreateAccessRequests` on `Client` — `client.ts:74`
...
### 23. Three Client methods, three different domain entity names — `client.ts:74,113,147`
...
### 24. `executeCall` / `executeHttpCall` naming pair — `utils.ts:26,65`
...
### 25. `HttpCallOptions` — `utils.ts:15`
...
### 27. `flattenQueryParams` — `utils.ts:123`
...
### 28. `PACKAGE_SEGMENT` constant — `client.ts:35`
...
### 30. `buildHttpRequest` parameter list — `utils.ts:96-102`
...
### 31. Loose typing for `executeCall(call, options)` `Options` field copying — `utils.ts:30-37`
...
### 32. `index.ts` is exhaustive but doesn't re-export schemas — `index.ts:1-24`
...
### 33. Comment-tag inconsistency — `client.ts:78,117,151` vs URL
...
`batchCreateAccessRequests`, `getAccessRequestDestinations`, `updateAccessRequestDestinations` — three different verbs across two resources. Verbs themselves match REST convention (`create`/`get`/`update`); the naming inconsistency is that the verb's target switches mid-class (see #23).
- **Category:** Observation.

### 36. `package.json` description is empty string — `package.json:4`
...
- Inferred but not in source: **`Terraform integration`** — appears in `AccessRequestDestinations.securableType` doc, suggests the redundant string fields exist because the Terraform provider can't read nested struct field types (see finding #5).
