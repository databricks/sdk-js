...
### 1. Package name `rfa` — `packages/uc/rfa/`, `package.json:2`, `client.ts:80,122,159`
...
### 2. Type name `AccessRequestDestinations` is plural — but represents ONE securable's destinations — `model.ts:54-73`
...
### 3. `BatchCreateAccessRequestsRequest` / `BatchCreateAccessRequestsResponse` — `model.ts:75,85`
...
### 4. Type name `SecurablePermissions` is plural but models ONE securable + its permissions — `model.ts:173-178`
...
### 5. `Principal` shape — bare `id` whose meaning depends on a sibling `principalType` — `model.ts:145-149`
...
### 6. Method `batchCreateAccessRequests` on `RfaClient` — `client.ts:76`
...
### 7. Three RfaClient methods, three different domain entity names — `client.ts:76,118,155`
...
### 8. Comment-tag inconsistency — `client.ts:80,122,159` vs URL
...
### 9. Action-verb conventions on `RfaClient`
`batchCreateAccessRequests`, `getAccessRequestDestinations`, `updateAccessRequestDestinations` — three different verbs across two resources. Verbs themselves match REST convention (`create`/`get`/`update`); the naming inconsistency is that the verb's target switches mid-class (see #7).
- **Category:** Observation.
