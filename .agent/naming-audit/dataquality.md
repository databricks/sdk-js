...
### 9. `DataProfilingStatus` has both `ERROR` and `FAILED` members; `DELETE_PENDING` word order — `src/v1/model.ts:57`
...
### 10. `Monitor.anomalyDetectionConfig` and `Monitor.dataProfilingConfig` — `src/v1/model.ts:399,404`
...
### 11. `NotificationSettings.onFailure: NotificationDestination` — `src/v1/model.ts:416`
...
### 12. Pervasive `Config` suffix on sibling domain types — `src/v1/model.ts:98,156,329,451,454`
